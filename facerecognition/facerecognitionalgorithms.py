...
def extract_embeddings(payload):
    ...
    if os.path.isdir("./facerecognition/random"):
        imagePaths = imagePaths + list(paths.list_images("./facerecognition/random"))
    print(imagePaths)
    ...
    for (i, imagePath) in enumerate(imagePaths):
        ...
        print("[INFO] processing image {}/{}".format(i + 1,
                                                     len(imagePaths)))
        userId = imagePath.split(os.path.sep)[-2]
        print(userId)
        ...
        if len(detections) > 0:
            ...
            if var_confidence > confidence:
                ...
                knownUserIds.append(userId)
                knownEmbeddings.append(vec.flatten())
                total += 1

    # dump the facial embeddings + names to disk
    print("[INFO] serializing {} encodings...".format(total))
    data = {"embeddings": knownEmbeddings, "userIds": knownUserIds}
...
def train_model(payload):
    ...
    le = LabelEncoder()
    labels = le.fit_transform(data["userIds"])
